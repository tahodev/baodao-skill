...
1. **當年度（115 年＝2026 年）：官方 xlsx 月曆**
   ...
   2026-09-12 實測：HTTP 200、19,770 bytes、`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`。放假日以**紅色字體**標示（週六、週日與國定假日皆紅），補班日以週六欄位的黑字表示。
...
RED={'31','34','38'}      # 紅字（放假）儲存格樣式 id；一般上班日是 '9'
out=[]
...
2026-09-12 實測：115 年檔案解析出 365 天，星期全部與實際日期吻合；平日放假日共 16 天（1/1、2/16-20 春節連假、2/27、4/3、4/6、5/1、6/19、9/25、9/28、10/9、10/26、12/25）。**月份不能讀標題文字**：十一月、十二月的「十」「一」分在兩個儲存格，直接用 `band*3+block+1` 推算最穩。補班日 = 星期欄是週六/週日但**不是**紅字的儲存格（115 年：無；若當年有，會出現在清單裡）。
...
awk -F, 'NR>1 && $4=="補行上班" {print $1, $2}' /tmp/cal114.csv  # 補班日
```

CSV 為 UTF-8（含 BOM）。`是否放假`：0=上班、2=放假；補班週六是 0 且備註「補行上班」。

## 錯誤與失敗時的處理

- **xlsx 連結 404**：人事總處每年換檔案 GUID。回到列表頁 https://www.dgpa.gov.tw/informationlist?uid=30 找當年度「政府行政機關辦公日曆表」公告，取其 xlsx 附件網址替換；解析邏輯不變，但要重驗 `HDR`（含「月」的列號）與 `RED`（紅字樣式 id）兩個常數——驗法：1 月 1 日必須是紅字星期四（以該年實際星期為準）。
...
Looks up Taiwan's official government work calendar (DGPA): national holidays, long weekends, and makeup workdays. Keyless. Current year (ROC 115 = 2026) ships as a red-ink xlsx calendar - download the dgpa.gov.tw xlsx and parse it with the python3-stdlib recipe in this file (365 days verified 2026-09-12; holidays are red-font cells, month = band*3+block+1). ROC 114 (2025) and earlier ship as structured CSV (西元日期,星期,是否放假,備註; 0=workday, 2=off, makeup Saturdays are 0 + 補行上班) from data.gov.tw dataset 14718. School/bank/stock-market calendars differ - do not mix.
