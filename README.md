...
只使用官方 API 與公開資料:財政部電子發票整合服務平台、臺北市資料大平臺、新北市資料開放平臺、台中市政府資料開放平臺、桃園市政府資料開放平臺、YouBike 微笑單車公開資料、Open-Meteo、中央氣象署開放資料平臺、國家災害防救科技中心（NCDR CAP）。以不需要登入、不需要 API 金鑰就能安全使用的查詢型技能為核心;少數技能（cwa-weather）使用免費、即時發給的 API 金鑰。
...
| YouBike 站點即時查詢 | `youbike-realtime` | 台北市+新北市+台中市+桃園市 YouBike 2.0 站點的可借車輛、可還空位即時數量（高雄等其他縣市未支援） | 不需要 | [youbike-realtime 指南](docs/features/youbike-realtime.md) |
...
| Look up YouBike station availability | `youbike-realtime` | Real-time rentable bikes and return docks for Taipei, New Taipei, Taichung, and Taoyuan YouBike 2.0 stations (Kaohsiung and others not yet supported) | Not required | [youbike-realtime guide](docs/features/youbike-realtime.md) |
